...
public class AlarmSyncReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context ctx, Intent intent) {
		 ...
		 */
		if (intent.getStringExtra("ID") == Setting.get(ctx, Setting.ALARM_AUTO_SYNC_LAST_ID)) {
			return;
		}
		...
		NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(ctx).setSmallIcon(R.drawable.ic_sync)
				.setContentTitle(rsc.getString(R.string.app_name)).setContentText(rsc.getString(R.string.auto_syncing));

		Intent resultIntent = new Intent(ctx, PengaturanSinkronasiActivity.class);

		PendingIntent resultPendingIntent = PendingIntent.getActivity(ctx, Setting.PI_ID_NOTIF_AUTO_SYNC, resultIntent,
		...
		// Toast.makeText(ctx.getApplicationContext(), "[DaftarTugas] Sinkronasi
		// otomatis", Toast.LENGTH_LONG).show();
	}
	...
	public static void setAutoSync(Context ctx) {
		...
		calendar.set(Calendar.HOUR_OF_DAY, Setting.getAsInt(ctx, Setting.AUTO_SYNC_TIME_HOUR));
		calendar.set(Calendar.MINUTE, Setting.getAsInt(ctx, Setting.AUTO_SYNC_TIME_MINUTE));
		...
		PendingIntent alarmIntent = PendingIntent.getBroadcast(ctx, Setting.PI_ID_AUTO_SYNC, intent, 0);
