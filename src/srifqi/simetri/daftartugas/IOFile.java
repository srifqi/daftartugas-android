...
public class IOFile {
	...
	/*
	 * // Not really needed for now. public static String eread(Context ctx,
	 * String dirpath, String path) { File dir = new File(
	 * Environment.getExternalStorageDirectory() + "/" + dirpath );
	 * dir.mkdirs(); File file = new File(dir, path); StringBuilder ftext = new
	 * StringBuilder();
	 * 
	 * try { BufferedReader br = new BufferedReader(new FileReader(file));
	 * String line;
	 * 
	 * while((line = br.readLine()) != null) { ftext.append(line);
	 * ftext.append("\n"); } try { ftext.deleteCharAt(ftext.length()-1); } catch
	 * (StringIndexOutOfBoundsException e) { // Empty string? }
	 * 
	 * br.close(); } catch (IOException e) { // Failed to read }
	 * 
	 * return ftext.toString(); }
	 * 
	 * public static boolean ewrite(Context ctx, String dirpath, String path,
	 * String text) { File dir = new File(
	 * Environment.getExternalStorageDirectory() + "/" + dirpath );
	 * dir.mkdirs(); File file = new File(dir, path);
	 * 
	 * FileOutputStream outstream;
	 * 
	 * try { outstream = new FileOutputStream(file);
	 * outstream.write(text.getBytes()); outstream.close(); } catch (Exception
	 * e) { // Failed to save return false; }
	 * 
	 * return true; }
	 */
}